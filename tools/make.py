...
''' % (__appdesc__, __copyright__)
	...
	def build_tools(self, tools, msg=True, debug=False):
		...
		for tool in tools:
			...
			command = "venv\\Scripts\\activate.bat & pyinstaller {0} --onefile	-p venv/Lib/site-packages/ --hidden-import=_cffi_backend --hidden-import=pylzma".format(tool)
			ret = subprocess_cmd(command)
